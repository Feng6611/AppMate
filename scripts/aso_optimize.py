...
def render(results: list[dict[str, Any]], data_today: dt.date) -> str:
    ...
    for idx, R in enumerate(results, 1):
        ...
        lines.append("")
        lines.append(f"**主市场**: {flag} {R['country']} (`{R['locale']}`)  ·  近 30 日下载 **{R['downloads_30d']:,}**")
        if not R["is_localized"]:
            lines.append("")
            lines.append(f"> ⚠️ 该 app 没有 **{R['country']}** 对应语言族的本地化 — 这是一个 ASO 空白机会")
        lines.append("")
        lines.append(f"- 主标题: `{R['title'] or '—'}`")
        ...
        lines.append("### 词集合分析（按 popularity 降序）")
        lines.append("")
        lines.append("| 关键词 | 来源 | 热度 | 难度 | 竞品 | 我的排名 | 建议 |")
        lines.append("|---|:-:|:-:|:-:|:-:|:-:|---|")
        ...
        lines.append("### 优化建议")
        lines.append("")
        lines.append(f"- 🔥/✅ **保留核心** ({len(keep)}): " + (", ".join(f"`{k}`" for k in keep) or "—"))
        lines.append(f"- ➕ **建议加入** ({len(add)}): " + (", ".join(f"`{k}`" for k in add) or "—"))
        lines.append(f"- 🎯 **攻坚（高热度未排好）** ({len(aspire)}): " + (", ".join(f"`{k}`" for k in aspire) or "—"))
        lines.append(f"- 🔁 **文案需调** ({len(fix)}): " + (", ".join(f"`{k}`" for k in fix) or "—"))
        lines.append(f"- ❌ **建议移除** ({len(drop)}): " + (", ".join(f"`{k}`" for k in drop) or "—"))
        lines.append("")
        lines.append("---")
        lines.append("")
